(() => {
  ...
  const sidebarTrigger = document.getElementById('rail-trigger');
  const sidebar = document.getElementById('publication-rail');
  const sidebarClose = document.getElementById('rail-close');
  const mask = document.getElementById('rail-scrim');
  ...
  document.querySelectorAll('#publication-rail a').forEach((link) => link.addEventListener('click', () => closeNavigation()));
  ...
  if (searchTrigger && searchPanel && typeof window.fetch === 'function') {
  ...
  const toc = document.getElementById('article-outline');
  if (article && toc) {
    ...
    if (headings.length) {
      ...
      toc.append(list);
      toc.closest('.hx-toc').hidden = false;
      toc.closest('.site-content-grid')?.classList.add('outline-ready');
    } else {
      toc.closest('.hx-toc')?.remove();
    }
  }

  document.querySelectorAll('.hx-article-body pre').forEach((block) => {
  ...
  if (copyLink && navigator.clipboard?.writeText) copyLink.hidden = false;
  copyLink?.addEventListener('click', async () => {
    await navigator.clipboard.writeText(window.location.href);
    copyLink.textContent = copyLink.dataset.success;
  });

  const backToTop = document.getElementById('page-top');
