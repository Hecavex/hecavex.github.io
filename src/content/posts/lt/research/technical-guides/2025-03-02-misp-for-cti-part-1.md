...
[MISP](https://www.misp-project.org/) – atvirojo kodo grėsmių informacijos platforma, padedanti organizacijoms struktūruotai rinkti, saugoti ir dalytis informacija apie kenkėjišką programinę įrangą, grėsmes ir pažeidžiamumus. CTI procese ji gali veikti kaip centrinė kompromitavimo indikatorių – IP adresų, domenų, failų maišų ir atakos požymių – saugykla bei koreliacijos variklis.

Prie šio apibrėžimo reikia vieno įspėjimo: MISP nėra visa žvalgybos programa vienoje dėžutėje. Blogus duomenis ji saugos taip pat efektyviai kaip gerus. Komandai vis tiek reikia žvalgybos reikalavimų, šaltinių vertinimo, pasitikėjimo, galiojimo terminų, atsakomybės ir vartotojų, galinčių veikti.
...
3. **Atsisiųskite ir paleiskite konteinerius.**
   ...
   Patikrinkite konteinerių būseną su `docker compose ps`, naujausius žurnalus su `docker compose logs --tail=200` ir pakeiskite kiekvieną numatytąjį arba sugeneruotą secret prieš atverdami sistemą išoriniam tinklui.
