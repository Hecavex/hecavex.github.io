---
title: "Adform Supply Chain Attack: JavaScript Crypto Clipper Analysis"
card_title: "Adform Supply Chain: Crypto Address Replacement in the Browser"
description: "Technical analysis of the Adform JavaScript supply-chain attack: four payload variants, exact-hash exposure across 59 hosts, cache replay, wallets and hunting rules."
date: 2026-08-08 10:30:00 +0300
last_modified_at: 2026-08-08 10:30:00 +0300
lang: en
translation_key: adform-supply-chain-crypto-clipper
permalink: /en/research/adform-supply-chain-crypto-clipper/
author: deividas-lis
content_type: malware-analysis
confidence: high
tlp: clear
categories: [malware, threat-intelligence, investigations]
tags: [Adform, supply-chain attack, JavaScript, crypto clipper, threat hunting, blockchain, Bitcoin, Ethereum]
featured: false
draft: false
toc: true
comments: false
scope: "Static analysis of the 26–27 July 2026 Adform Site Tracking JavaScript compromise, passive exposure hunting, blockchain analysis of the associated wallets and defensive hunting guidance."
limitations: "This analysis relies on public samples, web archives, URLScan and public blockchains. It does not establish every affected page or visitor, confirmed stolen funds, or attribution to a named threat actor."
key_findings:
...
image:
  ...
  alt: "A trusted third-party JavaScript file fans out to websites while a browser replaces a cryptocurrency destination address with an attacker's wallet"
...
And no, this was not the movie-version malware that installs itself in `System32`, turns the lights off and leaves a skull on the screen. According to [Adform's incident update](https://site.adform.com/resources/newsroom/security-incident-company-update/), the code did not persist on the device and only operated while an affected page remained open. That is still more than enough time for one irreversible blockchain transaction.
...
In its [5 August 2026 update](https://site.adform.com/resources/newsroom/security-incident-company-update/), Adform states that the first malicious activity began at **23:49 CEST on 26 July** (21:49 UTC), suspicious activity was detected at **03:00 CEST on 27 July** (01:00 UTC), and no new malicious-code distribution was observed after **19:16 CEST** (17:16 UTC).
...
![Timeline of the Adform incident and malicious JavaScript variants](/assets/img/posts/2026-08-08-adform-clipper/adform-incident-timeline.svg)
_Times are UTC. The blue accent on the final point represents a cached response, not proven continued attacker access._
...
[Adform's deployment documentation](https://www.adformhelp.com/hc/en-us/articles/10023216886545-Custom-Naming-JavaScript) says the tracking code can be used on one page, across several sections or throughout a website. Compromising the shared distribution file could therefore reach otherwise unrelated downstream pages. This is consistent with [MITRE ATT&CK T1195.002: Compromise Software Supply Chain](https://attack.mitre.org/techniques/T1195/002/).
...
- traversed DOM text nodes with `TreeWalker`
- inspected `input`, `textarea` and `contenteditable` elements
- overrode the `value` setters on `HTMLInputElement` and `HTMLTextAreaElement`
- hooked `copy`, `cut`, `paste` and `input` events
- repeated its checks every three seconds
- attempted to preserve cursor and selection positions after replacement.

![Adform crypto clipper attack chain](/assets/img/posts/2026-08-08-adform-clipper/adform-attack-chain.svg)
_Replacement could occur in displayed text, a form, a programmatic `value` write or the clipboard._
...
`84.32.102.230` falls within `84.32.102.0/24`. [RIPE RDAP](https://rdap.db.ripe.net/ip/84.32.102.230) shows an IPXO sub-allocated block, while [RIPEstat](https://stat.ripe.net/84.32.102.230) associates it with **AS59642 / UAB Cherry Servers**. The PTR was `ip-84-32-102-230.009.ptr.cherryservers.net`.
...
- [summary of all 59 hosts](/assets/data/adform-clipper-2026/observed-hosts.csv)
- [strict list of 55 hosts that received a functional BTC and ETH replacement variant](/assets/data/adform-clipper-2026/functioning-wallet-replacement-hosts.csv)
- [all 83 exact-hash URLScan sessions](/assets/data/adform-clipper-2026/urlscan-exact-hash-observations.csv), with timestamp, hostname, response SHA-256, payload capability and URLScan result link for every row
- [hash-to-capability matrix](/assets/data/adform-clipper-2026/payload-capabilities.csv), identifying the variants with functional BTC and ETH replacement addresses.

![URLScan screenshot of the Mindbank page](/assets/img/posts/2026-08-08-adform-clipper/urlscan-mindbank.png)
_URLScan captured `www.mindbank.ai` on 27 July 2026. The malware is not visible in the screenshot the technical evidence is the exact SHA-256 of `trackpoint-async.js` downloaded in that session. [Open the URLScan evidence](https://urlscan.io/result/019fa2e5-24ab-743f-8612-6159350af896/)._

![URLScan screenshot of the UBS page](/assets/img/posts/2026-08-08-adform-clipper/urlscan-ubs.png)
_URLScan captured a UBS page on 27 July 2026. The `s2.adform.net` resource in that session had the known `02ff86c7…` hash. This is an exposure signal, not proof of stolen funds. [Open the URLScan evidence](https://urlscan.io/result/019fa3c1-dcfd-73f4-a7ce-4983871fc0dc/)._

### The 30 July cache tail

The most interesting passive finding was not another domain but the response headers. In the [KICKS URLScan session](https://urlscan.io/result/019fb249-00f3-713f-910a-bdeaa0747b66/) at 09:09 UTC on 30 July 2026, the exact `02ff86c7…` variant was retrieved from `s1.matas.se/banners/scripts/st/trackpoint-async.js` with:
...
![URLScan screenshot of the KICKS page and cached Adform resource context](/assets/img/posts/2026-08-08-adform-clipper/urlscan-kicks-cached.png)
_The URLScan page image only provides session context. The primary finding is the exact response hash and cache metadata in the network log._
...
The address was active **before** the official Adform incident. It received 1.56008485 BTC on 28 July, including one inflow of 1.55751290 BTC, and 1.59736145 BTC was consolidated into `bc1qjeky…sglm7` on 29 July in [this transaction](https://mempool.space/tx/24eb95d4aabd7f31dbfec4637978cd7fa09ada4b0dccc063a6c42906a13c9d87).
...
Blockscout also shows that the address currently uses an [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702/) delegation to a publicly verified `TrustWallet.Biz.v1.0.0` implementation. That describes the smart-account mechanism. By itself, it does not suggest that Trust Wallet, the implementation author or their infrastructure participated in the operation.
...
- requests to `/banners/scripts/st/trackpoint-async.js` on `s2.adform.net` and custom CNAME hosts
- response-body SHA-256 matches for the four malicious hashes
- `ETag: "2c4f9056952481d2697d4e338b0caaf0"`
- HTTP requests to `84.32.102.230:7744` containing `/p?h=` and `&u=`
- cached copies in endpoints, web archives, service workers and browser-cache artifacts
...
1. Preserve retrieved `trackpoint-async.js` copies with response headers, timestamps and SHA-256 values.
2. Check origin, CDN, reverse proxy, custom CNAME, service-worker and browser-cache layers.
3. Purge the object across every cache-key variant, not just one URL.
4. Review CSP reports and proxy logs for `84.32.102.230:7744`.
5. Identify users and processes involved in cryptocurrency payments or wallet-address administration during the incident window.
6. Verify transaction recipient addresses through an independent channel, not the field displayed on screen at the time.
7. Rotate Adform integration credentials and review who could modify the distribution object where the integration is under your control.
8. Improve third-party JavaScript controls: CSP, Subresource Integrity where technically viable, self-hosting risk assessment and response-integrity monitoring.
...
1. [Adform: Security Incident, update of 5 August 2026](https://site.adform.com/resources/newsroom/security-incident-company-update/)
2. [Max Maass: captured `trackpoint-async.js` sample](https://gist.github.com/malexmave/8ef5eabc7b6866698f1ea8a811c75b57)
...
5. [Adform: Custom Naming JavaScript documentation](https://www.adformhelp.com/hc/en-us/articles/10023216886545-Custom-Naming-JavaScript)
...
9. [Mempool: BTC replacement address](https://mempool.space/address/bc1qmplgt0hcg62jc2guz86wn2sms7tqrsulkkrrls)
10. [Blockscout: ETH replacement address](https://eth.blockscout.com/address/0xE7983E69df17079ADb0aD7b3458488Cac0dBc573)
11. [EIP-7702: Set Code for EOAs](https://eips.ethereum.org/EIPS/eip-7702/)
12. [URLScan exact-hash search for `a04461bb…`](https://urlscan.io/search/#hash:a04461bbdccb15378182cdf77281ec29628f1c1386ae0fe89b62f359471fdeb6)
13. [URLScan cached `02ff86c7…` observation](https://urlscan.io/result/019fb249-00f3-713f-910a-bdeaa0747b66/)
